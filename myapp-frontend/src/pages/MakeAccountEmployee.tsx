...
export default function MakeAccountEmployee() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try{const res = await fetch(`${API_URL}/api/v1/employee_accounts`,{
        method: "POST",
        credentials: "include",
      ...
      if (res.ok){
        ...
        if (data.errors == "Email has already been taken"){
          setMail(true)
          toast("登録失敗", {
        description: "このEmailはすでに使用されています。",
        action: {
          label: "閉じる",
          onClick: () => {
            console.log("Undo")
        ...
        <Card className="p-8 shadow-2xl rounded-3xl">
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <Button
                type="submit"
                className="w-full mt-4 shadow-lg hover:scale-105 transition-transform"
              >
                アカウント作成
              </Button>
            </form>
